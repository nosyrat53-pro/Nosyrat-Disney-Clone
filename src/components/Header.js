...
const Header = (props) => {
    ...
    return (
        <Nav>
            ...
            {
                !props.userName || props.userName == "undefined" ? <Login onClick={() => handleAuth()}>LOGIN</Login> :

                    <>
                        <NavMenu>

                            <Link icon={homeicon} href="/home" text="HOME" />
                            <Link icon={searchicon} href="/search" text="SEARCH" />
                            <Link icon={watchlist} href="/watchlist" text="WATCHLIST" />
                            <Link icon={originals} href="/original" text="ORIGINALS" />
                            <Link icon={movies} href="/movie" text="MOVIES" />
                            <Link icon={series} href="/series" text="SERIES" />

                        </NavMenu>

                        <SignOut>
                            <UserImg src={props.userPhoto} />
                            <DropDown onClick={() => userSignOut()}>Sign Out</DropDown>
                        </SignOut>
                    </>
            }
        </Nav>
...
function mapDispatchToProps(dispatch) {
    return {
        addUserLoginData: () => dispatch(userLoginData),
...
const UserImg = styled.img`
    ...
    :hover{
        box-shadow: 0px 0px 6px rgba(200,200,200,.6)
    }
`;
...
const DropDown = styled.div`
    ...
    right: 15px;
    cursor: pointer;
    border: .5px solid rgba(151,151,151,.34);
    border-radius: 4px;
    display: none;
